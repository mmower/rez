...
And things are neater. Note that the component tag has a `.` prefix which is what Rez uses to identify `event_button` as a user component and look for its `@component` definition.
