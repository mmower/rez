...
This will be rendered as a link that looks for an `on_<event_name>` event handler attribute on the current card. The link syntax for loading new cards and scenes is transparently converted into event links with a built-in handler.
