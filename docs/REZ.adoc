...
Creating dynamic behaviour in Rez can be achieved by writing Javascript actions and event handlers. But this can become quite complex and Rez offers another way: behaviour trees.

Behaviour trees are a form of artificial intelligence that have been widely used to create realistic NPC behaviour in video games, starting with Halo 2 in 2004. They represent a more modular, flexible, and scalable approach to something like a finite state machine.
...
A behaviour tree is a kind of attribute, so it is specified as part of an element definition with a name (in this example `behaviours`), and is written in the form of a list `[...]` with a `^` prefix, i.e. `^[...]`.

Each individual behaviour has the form:

....
[element_id options* children*]
....

The `element_id` is the id of a `@behaviour` element implementing the required behaviour, for example `$sequence` or `actor_in`.

The use of a `$` prefix to the `element_id` indicates that it is one of the _core behaviours_ defined in the Rez stdlib. The `options` are zero or more key-value pairs separated with `=` (e.g. `actor = #sam_spade`), and `children` are zero or more other behaviours (specified as a list `[]` but without the `^` prefix).

In the example above there are four behaviours used: `$sequence`, `actor_in`, `item_in`, and `actor_drinks`. `$sequence` takes no options but has three children (`actor_in`, `item_in`, and `actor_drinks`), while `actor_in` takes 2 options (`actor` and `location`) but has no children.

The behaviours `actor_in`, `item_in`, and `actor_drinks` are not core behaviours so must be defined by the game author using the `@behaviour` element. As an author your job is to create behaviours that are meaningful for your game.
...
In our example above the `$sequence` is the _root behaviour_ so when we execute sam's `behaviours:` tree we are asking if the root `$sequence` succeeds or fails.

The xref:behaviour_catalog.adoc#_sequence[$sequence] behaviour succeeds or fails based on whether its children succeed or fail, so when it is asked to execute, it executes its children and succeeds or fails based on their own success or failure.

By contrast the xref:behaviour_catalog.adoc#_select[$select] behaviour executes its children until one succeeds, then it succeeds. It only fails if all of its children fail.

`$sequence` and `$select` are examples of _composite_ behaviours.
