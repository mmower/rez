...
There are two kinds of template expression.
...
At the moment there is no support for `else` requiring paired conditional templates for the `true` and `false` paths. This may be addressed in a future release.
