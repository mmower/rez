...
For data capture the simplest approach is to bind an `<input>` or `<select>` control to an attribute value using the `rez-bind` attribute.

....
<input type="text" rez-bind="player.name">
....

This sets up a two-way binding between the content of the `<input>` and the `player.name` attribute. Whatever is entered into the form input will be set directly on the `player.name` attribute. Equally assigning to the attribute `$("player").name = "..."` will update the input field.

This also works for checkboxes and boolean attributes.

....
<input type="checkbox" rez-bind="player.isOver18">
....

And for `<select>` controls:
...
At present radio inputs are not supported.
