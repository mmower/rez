...
In this example `#room_with_exits` has an `exits:` attribute and renders the card `#list_exits` as a block.

The `#list_exits` block doesn't have to know anything about `#room_with_exits` only that it defines an `exits:` attribute. We use a `binding_path` binding to refer to the `exits:` attribute in the source block of the parent block (i.e. in the card). This means we can also use `#list_exits` from `#another_room_with_exits` or any card that defines an `exits:` attribute.

=== What are Bindings?

By default the code being rendered in a block doesn't have access to all of your game objects.
...
|===
|`$block`
|the current renderer block object

|`card`
|the card that is being rendered

|`scene`
|if a scene layout is being rendered, the current scene object

|`game`
|if a game layout is being rendered, the game object
|===

In the previous example we used a `bindings:` attribute to make an `exits` value available to the template by referring to where the exits are defined. Bindings are powerful but complex and have some sharp edges.
