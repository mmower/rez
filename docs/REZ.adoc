...
The xref:behaviour_catalog.adoc#_sequence[$sequence] core behaviour succeeds or fails based on whether its children succeed or fail, so when it is asked to execute, it executes its children and succeeds or fails based on their own success or failure.
