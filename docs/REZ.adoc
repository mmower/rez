...
The HTML presented in the browser is generated from content template attributes defined (principally by your `@game`, `@scene`, and `@card` elements although there are others) glued together by markup provided by Rez.

At a high level the rendered structure looks like this:

`Game Layout`
&nbsp;&nbsp;`Scene Layout`
&nbsp;&nbsp;&nbsp;&nbsp;`Card(s)`

The `RezGame` renders its layout, which must include the `RezScene`. The `RezScene` layout renders one or more `RezCards`. A `RezCard` renders its content.

In practice it is a bit more complicated because each of these elements (including cards) can also render arbitrary other cards as part of their content. When a card is being included in this way it is refered to as a 'block' and is specified using the `blocks:` attribute of those elements. We'll come back to blocks in a moment.

The actual generation of markup is performed by a number of JS objects: `RezSingleLayout`, `RezStackLayout`, and `RezBlock`. Generating markup for `@game` and `@scene` layout is doing by one of `RezSingleLayout` or `RezStackLayout`. Generating markup for @cards is done by `RezBlock.` There is a 1:1 relationship between `@game` and `RezSingleLayout` and `@scene` and (of one) `RezSingleLayout` or `RezStackLayout`, and for each @card with an instance of `RezBlock`.

However the picture is slightly more complicated, because `@scene`'s and `@card`'s are considered to be templates and get copied before they are used. When you play a card such as `#fight_enemy` that card gets copied and it is the copy that is used. This allows for `@scene` and `@card` elements to be reused without having to worry about their condition. This is very important when a scene is used in stack mode.

All cards are rendered by a `RezBlock`.

=== What is a Block?

When we talk about a "block" in the rendering proces we're actually talking about a `@card` but a card intended to be included within other content. A classic example would be a sidebar that is common across a set of scenes.
...
@scene explore {
  blocks: [#sidebar]
  layout: ```
    scene layout content goes here
    ${sidebar}
    and continues here
  ```
}
...
The `blocks` attribute contains a list of `#id`'s of cards that are intended to be used as a block.

Note that when a card is used as in a block context it automatically gets a binding `$parent_block`. This allows the content of the block to easily reference attributes of the enclosing card.
...
In this example `#room_with_exits` has an `exits:` attribute and renders the card `#list_exits` as a block. The `#list_exits` block doesn't have to know anything about `#room_with_exits` only that it defines an `exits:` attribute. This means we can also use `#list_exits` from `#another_room_with_exits`.
...
....
<a href="javascript:void(0)" data-event="resume">Leave the store</a>
....
...
....
<a href="javascript:void(0)" data-event="reload">Reload gat</a>
....
...
....
<ul>
  <li><a href="javascript:void(0)" data-event="switch" data-target="conversation" data-actor_id="gutman">Speak with Gutman</a></li>
  <li><a href="javascript:void(0)" data-event="switch" data-target="conversation" data-actor_id="wilmer">Speak with Wilmer</a></li>
</ul>
....
