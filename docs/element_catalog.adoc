...
The `@inventory` element is used to describe containers.

An inventory is used to describe a container that can hold heterogenous items in slots.

For example, in an RPG it is common to have different slots to hold different types of
items like armour, a shield, melee weapon, range weapon, rings, and amulets. In other games you might have
slots describing wardrobe items. We can also think of memory (things we know about)
as an inventory (where the items are topics we know about) or spell books (where items
are individual spells). The `@inventory` element is designed to handle such cases.

When defining an `@inventory` you use the `slots:` attribute to define which slots the
inventory contains. This attribute is specified using a binding list (list of prefix `:` value pairs) where the prefix is a 'slot specification' and the value is the `#id` of an existing
`@slot` element. This allows us to re-use a slot.

The `@inventory` knows about specifiers describing slots.

....
....

 using a binding list
where the prefix is an inventory
...
|`effect_ids`
|List
|a list of id of `@effect`s that are active when this item is in an effects enabled slot
...
Called when the plot is started.

==== on_plot_did_advance

: `(plot, {stage: stage}) => {...}`

Called whenever the plot has advanced by 1 or more stages.
...
accept, and complete. It further supports turning in to a quest-giver as well at becoming
botched (incompletable) and unbotched.
...
|`excludes`
|Set of Slot Refs
|set of `@slot` ids that are mutually exclusive with this slot — if any position using an excluded slot type is occupied, this slot cannot accept items (and vice versa)
|===
