...
class RezBasicObject {
  ...
  initDynamicAttributes({generative = true} = {}) {
      ...
      if(typeof value === "object") {
        if(Object.hasOwn(value, "initializer") || Object.hasOwn(value, "dieroll")) {
          ...
          if(typeof(value.init) === "undefined") {
            copy_initializers[prio-1].push([attrName, value, null]);
          } else {
            const init_f = new Function("copy", value.init);
            copy_initializers[prio-1].push([attrName, value, init_f]);
          }
    ...
    if(generative) {
      copy_initializers.flat().forEach(
        ([attrName, elem_ref, init_f]) => {
          this.createAttributeByCopying(attrName, elem_ref, init_f);
        }
      );
    }
   ...
   */
  createAttributeByCopying(attrName, value, init_f) {
    const elem_ref = value.$copy;
    const source = $(elem_ref);
    this.setAttribute(attrName, source.addCopy(undefined, init_f).id);
  }
