...
A language, compiler, and runtime system for creating choice-based HTML interaction fiction
...
Rez is a tool for creating sophisticated choice-based HTML interactive fiction. These are games that use HTML/CSS, Javascript, images, movies, and sounds to present an interface and where players interact with the game through choices presented via HTML elements like links & form inputs.

Rez has excellent support for good looking & usable interfaces. It also builds in asset management, actors & behaviours, item & inventory management, factions, relationships, scenes/card structure, plots, customisable systems, and more.

Rez UI are built on top of the https://bulma.io/[Bulma] CSS framework and https://alpinejs.dev/[Alpine.JS] for modern layout and dynamic content.
...
The ideal Rez user is likely creating a game that is over-ambitious for Twine and is not afraid to learn to write simple Javascript functions.
