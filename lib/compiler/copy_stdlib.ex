defmodule Rez.Compiler.CopyStdlib do
  ...
  require EEx
  EEx.function_from_file(:def, :render_index, Path.expand("assets/templates/stdlib.rez.eex"), [:assigns])
  ...
  def run_phase(%Compilation{status: :ok, game: game, progress: progress} = compilation) do
    ...
    case File.write(output_path, source) do
      ...
      {:error, code} -> IOError.file_write_error(compilation, code, @stdlib_file_name, output_path)
    end
  end
