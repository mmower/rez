defmodule Rez.Compiler.CreateRuntime do
  ...
  EEx.function_from_file(:def, :render_runtime, Path.expand("assets/templates/runtime.js.eex"), [:assigns])

  @doc """
  Runs the game runtime template over the Game AST node.
  """
  def run_phase(
        %Compilation{status: :ok, game: game, dist_path: dist_path, progress: progress, options: %{output: true}} =
          compilation
      ) do
